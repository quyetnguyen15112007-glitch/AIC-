...
def chat_with_gpt():
    if not client:
        return jsonify({"error": "OpenAI client không được khởi tạo."}), 500

    data = request.json
    user_message = data.get('message')

    if not user_message:
        return jsonify({"error": "Không tìm thấy tin nhắn trong yêu cầu."}), 400

    try:
        completion = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Bạn là một trợ lý AI hữu ích. Luôn luôn trả lời bằng tiếng Việt, bất kể người dùng hỏi bằng ngôn ngữ nào."},
        ...
        print(f"Đã có lỗi xảy ra với OpenAI API: {e}")
        return jsonify({"error": "Đã có lỗi xảy ra khi giao tiếp với AI."}), 500
